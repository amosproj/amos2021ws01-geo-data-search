...
const SearchResultDetail = ({ result, onBackClick }: Props) => {
  return (
    <div className='sm:mt-4'>
      <div className="flex">
        <button aria-label='Close current search result' className="pr-2 py-2" onClick={onBackClick}>
          <ChevronLeftIcon className="h-6 w-6" />
        </button>
        <div>
          <h2 data-testid='current-search-result-name' className="font-bold text-2xl">{result.name}</h2>
